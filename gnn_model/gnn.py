...
class Model(torch.nn.Module):
    def __init__(self, args, concat=False):
        ...
        # Initialize the appropriate convolutional layer
        if self.model == 'gcn':
        ...
        # Additional layers for concatenation option
        if self.concat:
    ...
    def forward(self, data):
        ...
        # Optional concatenation with original features
        if self.concat:
            news = torch.stack([data.x[(data.batch == idx).nonzero().squeeze()[0]] 
...
def main():
    ...
    parser.add_argument('--feature', type=str, default='bert', help='feature type')
    parser.add_argument('--model', type=str, default='sage', help='model type')
    ...
    dataset = FNNDataset(root='data', feature=args.feature, empty=False,
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for data in train_loader:
            data.to(device)  # Move data to device (CPU or GPU)
            optimizer.zero_grad()
            out = model(data)
            y = data.y
        ...
        print(f'Epoch {epoch}: Loss: {loss_train:.4f}, Acc: {acc_train:.4f}, '
              f'Recall: {recall_train:.4f}, AUC: {auc_train:.4f}')
